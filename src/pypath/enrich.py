...
class Enrichment(object):
    ...
    def __init__(self, set_count, pop_count, set_size, pop_size, data):
        for k, v in locals().iteritems():
            setattr(self, k, v)
        self.pvalue = fisher.pvalue_population(set_count, set_size, 
            pop_count, pop_size).right_tail
...
class EnrichmentSet(object):
    ...
    def __init__(self, data, pop_size, 
        correction_method = 'hommel', alpha = 0.05):
        '''
        @data : dict
        ...
        for k, v in locals().iteritems():
            setattr(self, k, v)
        self.enrichments = OrderedDict(sorted([(name, 
            Enrichment(vals[0], vals[1], vals[2], self.pop_size, data = vals[3:])) \
            for name, vals in data.iteritems()], key = lambda x: x[0]))
        self.correction(method = correction_method, alpha = alpha)
    
    def correction(self, method = 'hommel', alpha = None):
        if alpha is None: alpha = self.alpha
        pvals_corr = smm.multipletests([enr.pvalue for enr in self.enrichments.values()], 
            alpha = alpha, method = method)[1]
