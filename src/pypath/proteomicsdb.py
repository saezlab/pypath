...
class ProteomicsDB(object):
    ...
    def __init__(self, username, password, output_format = 'json'):
        ...
        self.urls = {
            ...
            'proteinpertissue': 'https://www.proteomicsdb.org/proteomicsdb/logic/api/'\
    ...
    def query(self, api, param, silent = True, large = False):
        ...
        # long timeout is given, because huge files (hundreds MB) take time to load
        data = dataio.curl(url, req_headers = self.auth, silent = silent, 
            timeout = 1200, large = large)
    ...
    def get_proteins(self, tissue_id, calculation_method = 0, 
        swissprot_only = 1, no_isoform = 1):
        '''
        '''
        for i in xrange(3):
            self.query('proteinpertissue', (tissue_id, calculation_method, 
                swissprot_only, no_isoform, self.output_format), large = True)
                    ...
                    for p in pp:
                        ...
                        self.expression[protein['SAMPLE_NAME']]\
                            ...
                            float(non_digit.sub('', protein['NORMALIZED_EXPRESSION']))\
                                if normalized else \
                            float(non_digit.sub('', protein['UNNORMALIZED_EXPRESSION']))
                self.result.close()
                self.result = None
        except:
    ...
    def tissues_x_proteins(self, normalized = True, tissues = None):
        ...
        '''
        self.get_tissues()
        tissues_selected = set([t['TISSUE_ID'] for t in self.tissues \
            if tissues is None or t['TISSUE_ID'] in tissues]) - self.tissues_loaded
        prg = Progress(len(tissues_selected), 'Downloading expression data', 
            1, percent = False)
        for tis in tissues_selected:
            ...
            if type(self.result) is not file:
                ...
                if tis not in self.samples:
                    self.samples[tis] = []
                self.samples[tis] = uniqList(self.samples[tis] + \
                    list(self.current_samples))
                self.current_samples = set([])
        prg.terminate()
    
    def save(self, outf = None):
        self.result = None
        outf = outf if outf is not None else os.path.join('cache', 'proteomicsdb.pickle')
