...
class HLC:
    ...
    def merge_comms(self, edge1, edge2, S, dendro_flag=False):
        ...
        if dendro_flag:
            ...
            self.cid2edges[newcid] = self.cid2edges[cid1] | self.cid2edges[
                cid2]
            ...
            for e in self.cid2edges[
                    cid2]:  # move edges,nodes from cid2 to cid1
...
def similarities_weighted(adj, ij2wij):
    ...
    for n in adj:
        ...
        # includes (n,n)!
        n2a_sqrd[n] = sum(Aij[swap(n, i)]**2 for i in i_adj[n])
    ...
    for ind, n in enumerate(adj):  # n is the shared node
        ...
        if len(adj[n]) > 1:
            ...
            for i, j in combinations(adj[n], 2):
                ...
                ai_dot_aj = 1.0 * sum(Aij[swap(i, x)] * Aij[swap(j, x)]
                                      for x in inc_ns_i & inc_ns_j)

                # tanimoto similarity
                S = ai_dot_aj / (n2a_sqrd[i] + n2a_sqrd[j] - ai_dot_aj)
                heappush(min_heap, (1 - S, edge_pair))
    # return ordered edge pairs
    return [heappop(min_heap) for i in xrange(len(min_heap))]
...
def read_edgelist_weighted(filename,
                           delimiter=None,
                           nodetype=str,
                           weighttype=float):
    ...
    for line in open(filename, 'U'):
        L = line.strip().split(delimiter)
        ni, nj, wij = nodetype(L[0]), nodetype(L[1]), weighttype(
            L[2])  # other columns ignored
        if ni != nj:  # skip any self-loops...
            ni, nj = swap(ni, nj)
            edges.add((ni, nj))
            ij2wij[ni, nj] = wij
...
def write_edge2cid(e2c, filename, delimiter="\t"):
    ...
    # renumber community id's to be sequential, makes output file
    # human-readable
    # ugly...
    c2c = dict((c, i + 1)
               for i, c in enumerate(sorted(list(set(e2c.values())))))
    ...
    cid2edges, cid2nodes = defaultdict(set), defaultdict(
        set)  # faster to recreate here than
    ...
    f, g = open(filename + ".comm2edges.txt",
                'w'), open(filename + ".comm2nodes.txt", 'w')
...
"""
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="recording the whole dendrogram (optional)")
    ...
    if threshold is not None:
        if is_weighted:
            edge2cid, D_thr = HLC(adj, edges).single_linkage(
                threshold, w=ij2wij)
        else:
        ...
        if is_weighted:
            edge2cid, S_max, D_max, list_D = HLC(
                adj, edges).single_linkage(w=ij2wij)
        else:
            if dendro_flag:
                edge2cid, S_max, D_max, list_D, orig_cid2edge, linkage = HLC(
                    adj, edges).single_linkage(dendro_flag=dendro_flag)
                write_dendro("%s_dendro" % basename, orig_cid2edge, linkage)
            else:
                edge2cid, S_max, D_max, list_D = HLC(adj,
                                                     edges).single_linkage()
