...
class GDSC(object):
            ...
            for l in f:
                ...
                if not l[5].startswith('syn'):
                    ...
                    if len(mutation) != 0:
                        ...
                        mtd = intera.Residue(
                            mutation[1], mutation[2], uniprot, mutated=True)
    ...
    def get_microarray(self,
        ...
        prg = progress.Progress(
            len(rownames), 'Reading transcriptomic data', 1)
        for l in tbl:
            prg.step()
            ...
            if colnums is None and header is not None:
                colnums = dict([(cosmic, header.index(cosmic) + 1)
                                for cosmic in cosmix
                                if len(cosmic) > 0 and cosmic in header])
