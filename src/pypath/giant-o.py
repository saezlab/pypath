...
class Giant(object):
    ...
    def network(self, graph, tissues = None, cache_file = 'giant_cache.pickle', mapper = None):
            ...
            for e in random.sample(graph.es, graph.ecount()):
                if e['giant'][self.tissnm[tissue]] is None:
                    ...
                    if value is None:
                        ...
                        entrez = [self.ventrez[e.source], self.ventrez[e.target]]
                        if all(entrez):
                            #print '\tUniProts successfully translated to Entrez: %s. starting query' % str(entrez)
                            prg.step(eready - prev_ready, status = 'downloading (waiting for remote), iter: %u'%inum)
                            #print '\tquery params: %s, %s' % (str(tissue), str(entrez))
                            ...
                            #    0 if type(data) is not dict else len(data['edges']))
                            ...
                            for gedge in data['edges']:
                                ...
                                # print 'looking up edges %s and %s' % (str(epair), str(repair))
                            ...
                            pass
                            #print '\tsome of the UniProts could not be translated to Entrez'
                    else:
