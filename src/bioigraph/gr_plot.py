...
def meta_weight(meta,interGroup):
    ...
    for e in meta.es:
        ...
        e["weight"] = e["weight"] / (math.log(float(min(degA, degB)),2) + 1.0) ** 2
...
def meta_layout(g,groups,layout,**kwargs):
    ...
    if layout in set(["fr","fruchterman_reingold"]):
        ...
        meta.lo = meta.layout(
            "fr", weights=meta.es["weight"],
            repulserad=meta.vcount() ** 2, **kwargs)
    else:
    ...
    for i, gr in enumerate(subGraphs):
        ...
        loo = layout_norm(lo,
            [float(cntr[0]-dist[i]), float(cntr[0]+dist[i])],
...
def layout_intergroup(g,groups,**kwargs):
    ...
    lo = g.layout(
            "fr", weights=g.es["layout_weight"],
            repulserad=g.vcount() ** 2.4, **kwargs)
...
def group_colors(g,groups,edges=False):
    ...
    if edges:
        ...
        for e in g.es:
            e[groups+"_color"] = colormix(
                g.vs[e.source][groups+"_color"],
