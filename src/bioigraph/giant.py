...
class Giant(object):
    ...
    def query(self, tissue, entrez):
        url = data_formats.urls['giant']['url'] % (tissue, 'entrez=%s'%'&entrez='.join([str(e) for e in entrez]))
        data = dataio.curl(url, silent = True, cache = False, debug = self.debug, override_post = True)
        if self.debug:
            self.trace['url'] = url
            self.trace['raw'] = data
    ...
    def network(self, graph, tissues = None, cache_file = 'giant_cache.pickle', mapper = None, size = 100):
        self.cache = cache_file
        self.load()
        self.init_mapper(mapper)
        if 'giant' not in graph.es.attributes(): graph.es['giant'] = [{} for _ in graph.es]
        tissues = self.tissid.keys() if tissues is None else tissues
        tissues = [int(t) if type(t) in charTypes and t.isdigit() and int(t) in self.tissnm else t for t in tissues]
        tissues = [self.tissid[t] if t in self.tissid else t for t in tissues]
        self.ventrez = [None if len(e) == 0 else e[0] for e in \
            [mapper.map_name(v['name'], 'uniprot', 'entrez') for v in graph.vs]]
        self.eentrez = dict(((self.ventrez[e.source], self.ventrez[e.target]), e.index) for e in graph.es)
        if len(set(tissues) - set(self.tissnm.keys())) > 0:
            sys.stdout.write('\t:: The following tissues could not be identified in GIANT tissues list:\n\t\t%s\n' % \
                str(list(set(tissues) - set(self.tissnm.keys()))))
            sys.stdout.flush()
        tissues = list(set(tissues) & set(self.tissnm.keys()))
        for tissue in tissues:
            for e in graph.es:
                if self.tissnm[tissue] not in e['giant']: e['giant'][self.tissnm[tissue]] = None
    ...
    def to_cache(self, data, tissue):
        if tissue not in self.cached: self.cached[tissue] = {}
        self.cached[tissue][tuple(sorted([data[0], data[1]]))] = data[2]

    def iterate(self, graph, tissue, size = 100):
        if self.debug: self.trace['edges_from_cache'] = []
        eready = 0
        prev_ready = -1
        prg = progress.Progress(graph.ecount(), 'Loading tissue specificity data for %s'%self.tissnm[tissue], 1)
        inum = 0
        while eready > prev_ready:
            inum += 1
            prg.step(eready - prev_ready, status = 'iteration %u'%inum)
            entlst = []
            if self.debug: self.trace['input_edges'] = []
            for e in graph.es:
                value = None
                if e['giant'][self.tissnm[tissue]] is None:
                    value = self.from_cache(graph.vs[e.source]['name'], graph.vs[e.target]['name'], tissue)
                if value is None:
                    if self.ventrez[e.source] is not None and self.ventrez[e.source] not in entlst:
                        entlst.append(self.ventrez[e.source])
                    if self.ventrez[e.target] is not None and self.ventrez[e.target] not in entlst: 
                        entlst.append(self.ventrez[e.target])
                    print 'length of query list: %u' % len(entlst)
                    if self.debug: self.trace['input_edges'].append(e.index)
                    if len(entlst) > size or (e.index == graph.ecount() - 1 and len(entlst) > 0):
                        print '\t:: starting query'
                        if self.debug: self.trace['entrez_list'] = entlst
                        prg.step(eready - prev_ready, status = 'downloading (waiting for remote), iter: %u'%inum)
                        data = self.query(tissue, entlst)
                        genes = self.genes_dict(data)
                        prg.step(eready - prev_ready, status = 'processing data, iter: %u'%inum)
                        if self.debug: self.trace['edges_set'] = []
                        for gedge in data['edges']:
                            # here we get the Gene IDs from the list indexes:
                            epair = (str(genes[gedge['source']]), str(genes[gedge['target']]))
                            ...
                            if eid is not None:
                                ...
                                graph.es[eid]['giant'][self.tissnm[tissue]] = gedge['weight']
                                self.to_cache([graph.vs[graph.es[eid].source]['name'], 
                                    graph.vs[graph.es[eid].target]['name'], gedge['weight']], tissue)
                                if self.debug: self.trace['edges_set'].append(eid)
                        prev_ready = eready
                        entlst = []
                else:
                    e['giant'][self.tissnm[tissue]] = value
                    if self.debug: self.trace['edges_from_cache'].append(e.index)
                eready = len([e for e in graph.es if e['giant'][self.tissnm[tissue]] is not None])
        prg.terminate()