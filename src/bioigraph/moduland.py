...
class Moduland(object):
    ...
    def __init__(self, network, directed = None, tmpdir = 'moduland_tmp', 
        ...
        if os.name != 'posix':
            sys.stdout.write('ModuLand executables can run only on POSIX'\
        ...
        self.landscape_commands = {
            ...
            'edgeweight': '%s' % (self.modexe['edgeweight']) + ' --in %s --out %s',
    ...
    def update_network(self, network, weight = None):
        ...
        if self.pajek is not None:
            self.seq += 1
            self.fname_stems.append(os.path.join(
                self.tmpdir,self.session + '-%03d'%self.seq))
    ...
    def pajek_from_edgelist(self):
        ...
        for e in network:
            if len(e) > 1:
                ...
                self.console('Edgelist contains element shorter than 2. '\
    ...
    def run_perturland(self, p = 0.01, recalculate = False):
        ...
        lscape_cmd['cmd'] = self.landscape_commands['perturland'] % (self.cxg, p, cxl)
    ...
    def run_landscape(self, landscape = 'linkland', p = 0.01, recalculate = False):
        if landscape == 'perturland':
            ...
            lscape_cmd['cmd'] = self.landscape_commands[landscape] % (self.cxg, cxl)
            lscape_cmd['inputs'] = [self.cxg]
            lscape_cmd['outputs'] = [cxl]
            lscape_cmd['recalculate'] = recalculate
            self.run_anything(lscape_cmd)
    ...
    def run_anything(self, param):
        if param['cmd'] in self.processes:
            if self.processes[param['cmd']]['process'].__class__.__name__ == 'Popen' \
                ...
                self.console('Won\'t start command:\n\t%s\nThe same command already'\
                    ' running, pid: %u' % \
                    (param['cmd'], self.processes[param['cmd']]['process'].pid))
                return None
            if self.processes[param['cmd']]['finished'] is not None \
                and not param['recalculate']:
                sys.stdout.write('Command already run and finished:\n'\
    ...
    def start_process(self, cmd):
        ...
        if cmd in self.processes and self.processes[cmd]['process'] is None:
            self.processes[cmd]['process'] = subprocess.Popen(cmd, 
            stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell = True)
