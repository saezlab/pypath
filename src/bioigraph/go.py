...
class GOEnrichmentSet(enrich.EnrichmentSet):
    ...
    def __init__(self, aspect, organism = 9606, annotation = None, basic_set = None, 
    ...
    def calculate(self):
        data = dict([(term, (cnt, self.counts_pop[term], self.set_size, \
            self.annotation.name[term])) for term, cnt in self.counts_set.iteritems()])
        enrich.EnrichmentSet.__init__(self, data, self.pop_size, alpha = self.alpha, 
            correction_method = self.correction_method)
    
    def get_basic_set(self):
        swissprots = set(dataio.all_uniprots(organism = self.organism, swissprot = 'yes'))
        return dict(filter(lambda x: x[0] in swissprots, 
            getattr(self.annotation, self.aspect.lower()).iteritems()))
    
    def get_annot(self, set_names):
        return dict(filter(lambda x: x[0] in set_names, self.basic_set.iteritems()))
